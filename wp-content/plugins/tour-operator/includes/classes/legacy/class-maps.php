...
class Maps {
	...
	public function get_map_preview_html( $width = '', $height = '' ) {
		...
		if ( '' !== $preview_src ) {
			...
			$preview_html .= '<div class="placeholder-text"><h2 class="lsx-to-section-title lsx-to-collapse-title" style="margin-top:-' . ( ( (int) $height / 2 ) + 31 ) . 'px">' . esc_html__( 'Click to display the map', 'tour-operator' ) . '</h2></div>';
		}
		return $preview_html;
	}
	/**
