...
class TO_Banners_Frontend extends TO_Banners {
	...
	public function banner() {
		...
		if ( ! empty( $show_slider ) || ! empty( $banner_image ) || ! empty( $embed_video ) || ! empty( $bg_color ) ) {
			add_filter( 'lsx_global_header_disable', '__return_true' );
			?>
			<div id="minimal-slider" class="minimal-slider">
				<?php
				...
				if ( empty( $show_slider ) ) {
						...
						<!--<div class="<?php echo esc_attr( $class_attr ); ?>"> -->
							<div class="minimal-slider-slide">
								...
								<?php if ( ! empty( $banner_image ) ) : ?>
									...
									<div class="minimal-slider-slide-content-wrapper minimal-slider-slide-content-wrapper-1" style="background-image:url(<?php echo esc_attr( $banner_image ); ?>);">
										<div class="minimal-slider-slide-content big-heading"></div>
									</div>
								<?php endif; ?>
					...
					?>
							...
							<?php foreach ( $img_group['banner_image'] as $key => $value ) {
								...
								?>
								<div class="minimal-slider-slide">
									<div class="minimal-slider-slide-content-wrapper minimal-slider-slide-content-wrapper-<?=$count;?>" style="background-image:url(<?php echo esc_attr( $slide[0] ); ?>);">
										<div class="minimal-slider-slide-content big-heading"></div>
									</div>
								</div>
	...
	public function banner_part_title( $post_title ) {
		...
		if ( is_post_type_archive( $allowed_post_types ) && isset( $this->options[ get_post_type() ] ) && isset( $this->options[ get_post_type() ]['title'] ) ) {
			...
			$post_title = '<h1 class="page-title aaaa">' . get_the_archive_title() . '</h1>';
		} elseif ( is_tax( $allowed_taxonomies ) || is_category() ) {
			$post_title = '<h1 class="page-title bbbb">' . single_term_title( '', false ) . '</h1>';
		} elseif ( is_author() ) {
			$post_title = '<h1 class="page-title cccc">' . get_the_archive_title() . '</h1>';
